...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " neon-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  neon-tx [options] <hex-tx> [commands]  Update hex-encoded neon transaction\n" +
            "or:     neon-tx [options] -create [commands]   Create hex-encoded neon transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded neon transaction
