...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount)
{
    ...
    entry->findChild<NeonAmountField*>("payAmount")->setValue(amount);
    uint256 txid;
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_neon-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_neon-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_neon-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = NeonUnits::formatWithUnit(unit, balance, false, NeonUnits::separatorAlways);
    ...
    // Amount input
    NeonAmountField* amountInput = receiveCoinsDialog.findChild<NeonAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QVERIFY(paymentTextList.at(1).indexOf(QString("URI: neon:")) != -1);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_neon-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
