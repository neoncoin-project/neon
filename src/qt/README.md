This directory contains the NeonQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Neon UI.
...
ClientModel is used by the main application `neongui` and several models like `peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a neon:... link from a browser).
...
* UI elements like NeonAmountField, which inherit from QWidget.
* `neonstrings.cpp`: automatically generated
* `neonunits.(h/cpp)`: NEON / mNEON / etc handling
* `callback.h`
...
* don't change `local/neon_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "neon-qt" under "Run", which is where you can also add command line arguments)
