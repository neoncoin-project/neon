...
{
    GUIUtil::setClipboard(NeonUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
//            }
            ...
            // neonoin: sub-cent change is moved to fee
...
//            }
    ...
    int nDisplayUnit = NeonUnits::NEON;
    ...
    l2->setText(NeonUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(NeonUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(NeonUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l8->setText(NeonUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            if(ExtractDestination(out.txout.scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show neon address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, NeonUnits::format(nDisplayUnit, out.txout.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, NeonUnits::format(nDisplayUnit, nSum));
