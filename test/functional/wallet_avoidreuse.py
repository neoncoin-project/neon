...
class AvoidReuseTest(NeonTestFramework):
    ...
    def test_fund_send_fund_senddirty(self):
        '''
        Test the same as test_fund_send_fund_send, except send the 10 NEON with
        the avoid_reuse flag set to false. This means the 10 NEON send should succeed,
        ...
        # listunspent should show 1 single, unused 10 neon output
        assert_unspent(self.nodes[1], total_count=1, total_sum=10, reused_supported=True, reused_count=0)
        # getbalances should show no used, 10 neon trusted
        ...
        # listunspent should show 1 single, unused 5 neon output
        assert_unspent(self.nodes[1], total_count=1, total_sum=5, reused_supported=True, reused_count=0)
        # getbalances should show no used, 5 neon trusted
        ...
        # listunspent should show 2 total outputs (5, 10 neon), one unused (5), one reused (10)
        assert_unspent(self.nodes[1], total_count=2, total_sum=15, reused_count=1, reused_sum=10)
        # getbalances should show 10 used, 5 neon trusted
        assert_balances(self.nodes[1], mine={"used": 10, "trusted": 5})

        self.nodes[1].sendtoaddress(address=retaddr, amount=10, avoid_reuse=False)

        # listunspent should show 1 total outputs (5 neon), unused
        assert_unspent(self.nodes[1], total_count=1, total_sum=5, reused_count=0)
        # getbalances should show no used, 5 neon trusted
        assert_balances(self.nodes[1], mine={"used": 0, "trusted": 5})

        # node 1 should now have about 5 neon left (for both cases)
    ...
    def test_fund_send_fund_send(self, second_addr_type):
        ...
        [0] sends 10 NEON to A.
        [1] spends 5 NEON from A. (leaving roughly 5 NEON useable)
        [0] sends 10 NEON to A again.
        [1] tries to spend 10 NEON (fails; dirty).
        [1] tries to spend 4 NEON (succeeds; change address sufficient)
        '''
        ...
        # listunspent should show 2 total outputs (1, 10 neon), one unused (1), one reused (10)
        assert_unspent(self.nodes[1], total_count=2, total_sum=11, reused_count=1, reused_sum=10)
        # getbalances should show 10 used, 1 neon trusted
        assert_balances(self.nodes[1], mine={"used": 10, "trusted": 1})

        # node 1 should now have about 1 neon left (no dirty) and 11 (including dirty)
