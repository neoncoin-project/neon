...
from test_framework.test_framework import NeonTestFramework, SkipTest
...
class BackwardsCompatibilityTest(NeonTestFramework):
    ...
    def setup_nodes(self):
        ...
        self.add_nodes(self.num_nodes, extra_args=self.extra_args, versions=[
            ...
            self.options.neond,
            self.options.neond,
            releases_path + "/v0.19.0.1/bin/neond",
            releases_path + "/v0.18.1/bin/neond",
            releases_path + "/v0.17.1/bin/neond"
        ], binary_cli=[
            self.options.neoncli,
            self.options.neoncli,
            releases_path + "/v0.19.0.1/bin/neon-cli",
            releases_path + "/v0.18.1/bin/neon-cli",
            releases_path + "/v0.17.1/bin/neon-cli"
        ])
    ...
    def run_test(self):
        ...
        # RPC loadwallet failure causes neond to exit, in addition to the RPC
        # call failure, so the following test won't work:
        # assert_raises_rpc_error(-4, "Wallet loading failed.", node_v17.loadwallet, 'w3_v18')

        # Instead, we stop node and try to launch it with the wallet:
        self.stop_node(self.num_nodes - 1)
        node_v17.assert_start_raises_init_error(["-wallet=w3_v18"], "Error: Error loading w3_v18: Wallet requires newer version of Neon Core")
        node_v17.assert_start_raises_init_error(["-wallet=w3"], "Error: Error loading w3: Wallet requires newer version of Neon Core")
        ...
        # Copy the 0.17 wallet to the last Neon Core version and open it:
        ...
        # Copy the 0.19 wallet to the last Neon Core version and open it:
        shutil.copytree(
