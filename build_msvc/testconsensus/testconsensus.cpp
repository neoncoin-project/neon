...
int main()
{
    std::cout << "neonconsensus version: " << neonconsensus_version() << std::endl;
    ...
    auto op0Result = neonconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, neonconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
