...
- [Development guidelines](#development-guidelines)
    - [General Neon Core](#general-neon-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Neon Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play neons" on the test network, if you
...
Neon is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/neon/neon/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_neon
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_neon --log_level=test_suite
$ valgrind -v --leak-check=full src/neond -printtoconsole
...
# A coverage report will now be accessible at `./test_neon.coverage/index.html`.
```
...
To profile a running neond process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep neond` -- sleep 60
```
...
Neon Core can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/neon/neon/issues/12691)
...
However, in open source software such as Neon, where everyone uses
...
pay attention to for reviewers of Neon code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Neon uses tinyformat, which is type safe. Leave them out to avoid confusion
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `NEON_FOO_BAR_H`, e.g.

```c++
#ifndef NEON_FOO_BAR_H
#define NEON_FOO_BAR_H
...
#endif // NEON_FOO_BAR_H
```
...
Some of these are maintained by active developers of Neon, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Neon so that they can be integrated
...
- src/secp256k1
  - Upstream at https://github.com/neon-core/secp256k1/ ; actively maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/neon-core/ctaes ; actively maintained by Core contributors.

- src/univalue
  - Upstream at https://github.com/neon-core/univalue ; actively maintained by Core contributors, deviates from upstream https://github.com/jgarzik/univalue
...
file descriptors, it will cause problems with Neon's `select()` loop, because
...
compatibility between nodes. This happened in Neon 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Neon's
...
- [scripted-diff: Rename InitInterfaces to NodeContext](https://github.com/neon/neon/commit/301bd41a2e6765b185bd55f4c541f9e27aeea29d)
uses an elegant script to replace occurrences of multiple terms in all source files.

- [scripted-diff: Remove g_connman, g_banman globals](https://github.com/neon/neon/commit/8922d7f6b751a3e6b3b9f6fb7961c442877fb65a)
replaces specific terms in a list of specific source files.

- [scripted-diff: Replace fprintf with tfm::format](https://github.com/neon/neon/commit/fac03ec43a15ad547161e37e53ea82482cc508f9)
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `neon-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Neon, the wallet was always
...
- For consistency and friendliness to code generation tools, interface method
  ...
  before in [#14635](https://github.com/neon/neon/pull/14635).
